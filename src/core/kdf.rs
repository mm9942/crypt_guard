...
impl KeyOperations for Falcon1024 {
    ...
    fn save_public(public_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Falcon1024/key.pub"),
    ...
    fn save_secret(secret_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Falcon1024/key.sec"),
...
impl SignatureFunctions for Falcon1024 {
    ...
    fn verify(signature: Vec<u8>, data: Vec<u8>, key: Vec<u8>) -> Result<bool, SigningErr> {
        ...
        match &data {
            ...
            false => log_activity!(
                "Verification failed! Please use for more infos: RUST_BACKTRACE=[1 or full]",
                "\nUsed key: Falcon1024"
            ),
        };
        Ok(data)
...
impl KeyOperations for Falcon512 {
    ...
    fn save_public(public_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Falcon512/key.pub"),
            FileTypes::public_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(public_key);
    ...
    fn save_secret(secret_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Falcon512/key.sec"),
            FileTypes::secret_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(secret_key);
...
impl KeyOperations for Dilithium2 {
    ...
    fn save_public(public_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium2/key.pub"),
            FileTypes::public_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(public_key);
    ...
    fn save_secret(secret_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium2/key.sec"),
            FileTypes::secret_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(secret_key);
...
impl KeyOperations for Dilithium3 {
    ...
    fn save_public(public_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium3/key.pub"),
            FileTypes::public_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(public_key);
    ...
    fn save_secret(secret_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium3/key.sec"),
            FileTypes::secret_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(secret_key);
...
impl KeyOperations for Dilithium5 {
    ...
    fn save_public(public_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium5/key.pub"),
            FileTypes::public_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(public_key);
    ...
    fn save_secret(secret_key: &[u8]) -> Result<(), SigningErr> {
        let file = FileMetadata::from(
            PathBuf::from("./Dilithium5/key.sec"),
            FileTypes::secret_key(),
            FileState::not_encrypted(),
        );
        let _ = file.save(secret_key);
