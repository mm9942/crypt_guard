use crate::{keychain::*, Keychain, SigningErr};
use hex;
use pqcrypto_dilithium::dilithium5::{self, *};
...
impl SignDilithium {
    ...
    pub async fn save_signed_msg(&self, base_path: &str, title: &str) -> Result<(), SigningErr> {
        if let Some(signed_msg) = &self.signed_msg {
            ...
            let signed_message_path =
                Keychain::generate_unique_filename(&format!("{}/{}", dir_path, title), "msg");
    ...
    pub async fn save_keys(&self, base_path: &str, title: &str) -> Result<(), CryptError> {
        ...
        let public_key_path =
            Keychain::generate_unique_filename(&format!("{}/{}", dir_path, title), "pub");
        let secret_key_path =
            Keychain::generate_unique_filename(&format!("{}/{}", dir_path, title), "sec");
