...
impl CipherAES {
    ...
    fn decryption(&mut self) -> Result<Vec<u8>, CryptError> {
        ...
        println!("data len: {}", encrypted_data_with_hmac.len());
...
impl CryptographicFunctions for CipherAES {
	...
	fn encrypt(&mut self, public_key: Key) -> Result<(Vec<u8>, Key), CryptError> {
		...
		println!("shared secret: {:?}\nLength: {}", sharedsecret.content()?.to_vec(), sharedsecret.content()?.to_vec().len());
		let _ = self.set_shared_secret(sharedsecret.content()?.to_vec());
