...
impl CipherChaCha {
	...
	pub fn new(infos: CryptographicInformation, nonce: Option<String>) -> Self {
        let nonce = match nonce {
            Some(nonce) => hex::decode(nonce).unwrap().try_into(),
            None => Ok(generate_nonce()),
        };
		CipherChaCha { infos, sharedsecret: Vec::new(), nonce: nonce.unwrap() }
	}
...
impl CryptographicFunctions for CipherChaCha {
    ...
    fn encrypt(&mut self, public_key: Key) -> Result<(Vec<u8>, Key), CryptError> {
        let (ciphertext, sharedsecret) = public_key.encap()?;        
        self.set_shared_secret(sharedsecret.content()?.to_vec());
        let (encrypted_data, nonce) = self.cryptography()?;
        println!("Please write down this nonce: {:?}", nonce);
    ...
    fn decrypt(&mut self, secret_key: Key, ciphertext: Key) -> Result<Vec<u8>, CryptError> {
		let sharedsecret = secret_key.decap(ciphertext)?;
        self.set_shared_secret(sharedsecret.content()?.to_vec());
